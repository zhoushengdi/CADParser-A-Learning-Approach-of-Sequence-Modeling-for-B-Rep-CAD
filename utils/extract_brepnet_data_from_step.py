...
from utils.face_index_validator import FaceIndexValidator
from utils.segmentation_file_crosschecker import SegmentationFileCrosschecker
...
class BRepNetExtractor:
    ...
    def process(self):
        ...
        ## Here face feature is just a one-hot vector on behalf of face type
        face_features = self.extract_face_features_from_body(body, entity_mapper)
        ### Here edge feature actually is two one-hot part, one for convexity, the other for Edge type
        edge_features = self.extract_edge_features_from_body(body, entity_mapper)
        ### Here co-edge feature is only one feature represent its orientation
        coedge_features = self.extract_coedge_features_from_body(body, entity_mapper)

        ### TopoDS_Compound部分的代码是shengdi_zhou写的,用于遍历每个单独的body,然后将所有的body的feature数据级联起来.
        if isinstance(body, TopoDS_Compound):
            face_point_grids_list = []
            coedge_point_grids_list = []
            coedge_lcs_list = []
            bodys = TopoDS_Iterator(body)
            offset_f = 0
            offset_c = 0
            offset_lcs = 0
            while(bodys.More()):
                body_s = bodys.Value()
                ## 返回的是一个[ num_faces x 7 x num_pts_u x num_pts_v ]的tensor,
                face_point_grids_s = self.extract_face_point_grids(body_s, entity_mapper, offset_f)
                assert face_point_grids_s.shape[1] == 7
                face_point_grids_list.append(face_point_grids_s)
                ## 返回的是一个[ num_coedges x 12 x num_u ]的tensor，其中12主要是代表相邻面的normal,边上的点的坐标以及tangent值
                coedge_point_grids_s = self.extract_coedge_point_grids(body_s, entity_mapper, offset_c)
                assert coedge_point_grids_s.shape[1] == 12
                coedge_point_grids_list.append(coedge_point_grids_s)
                ## 返回的是一个[4 x 4]的矩阵，表示从局部坐标到全局坐标的harmogenous矩阵
                coedge_lcs_s = self.extract_coedge_local_coordinate_systems(body_s, entity_mapper, offset_lcs)
                coedge_lcs_list.append(coedge_lcs_s)
                offset_f += face_point_grids_s.shape[0]
                offset_c += coedge_point_grids_s.shape[0]
                offset_lcs += coedge_lcs_s.shape[0]
                bodys.Next()
            face_point_grids = np.concatenate(face_point_grids_list, axis=0)
            coedge_point_grids = np.concatenate(coedge_point_grids_list, axis=0)
            coedge_lcs = np.concatenate(coedge_lcs_list, axis=0)
        else:
        ...
        ## coedge_reverse_flags应该是用来标记每个coedge的方向标记,按道理大小应该与coedge_point_grids_s.shape[0]一致.
    ...
    def extract_face_point_grids(self, body, entity_mapper, offset=0):
        """
        ...
        for face in solid.faces():
            assert len(face_grids)+offset == entity_mapper.face_index(face.topods_shape())
    ...
    def extract_face_point_grid(self, face):
        ...
        num_u=10
        num_v=10
        ### uvgrid是occwl中实现的函数，主要用于将通过face提取uv点出的point值，以及对应的normal，
        # 以及是否是body内部的label，所以最后维度的大小是7
    ...
    def extract_coedge_point_grids(self, body, entity_mapper, offset=0):
        """
        ...
        for wire in top_exp.wires():
            wire_exp = TopologyUtils.WireExplorer(wire)
            for coedge in wire_exp.ordered_edges():
                assert len(coedge_grids)+offset == entity_mapper.halfedge_index(coedge)
    ...
    def extract_coedge_local_coordinate_systems(self, body, entity_mapper, offset=0):
        """
        ...
        for wire in top_exp.wires():
            wire_exp = TopologyUtils.WireExplorer(wire)
            for coedge in wire_exp.ordered_edges():
                assert len(coedge_lcs)+offset == entity_mapper.halfedge_index(coedge)
                occwl_oriented_edge = Edge(coedge)
                faces = [ f for f in solid.faces_from_edge(occwl_oriented_edge) ]
...
def run_worker(worker_args):
    ...
    chunk_num = str(file).split('/')[-2]
    output_path_t = output_path / chunk_num
    if (not output_path_t.exists()):
        output_path_t.mkdir(parents=True, exist_ok=True)
    feature_schema = worker_args[2]
    mesh_dir = worker_args[3]
    seg_dir = worker_args[4]
    try:
        extract_brepnet_features(file, output_path_t, feature_schema, mesh_dir, seg_dir)
...
def filter_out_files_which_are_already_converted(files, output_path):
    ...
    for file in files:
        chunk_num = str(file).split('/')[-2]
        output_file = output_path / chunk_num / (file.stem + ".npz")
...
def extract_brepnet_data_from_step(
    ...
    files = [ f for f in step_path.glob("**/*.step")]
    # step_files = [ f for f in step_path.glob("**/*.step")]
    # files.extend(step_files)
    # files = [Path('../data/cad_step/0015/00158213.step')]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--step_path", type=str, default='../data/cad_step', help="Path to load the step files from")
    parser.add_argument("--output", type=str, default = '../data/step_feature', help="Path to the save intermediate brep data")
    parser.add_argument("--feature_list", type=str, default='../data/featurelist/all.json',help="Optional path to the feature lists")
    parser.add_argument("--num_workers", type=int, default=10, help="Number of worker threads")
    parser.add_argument(
        "--mesh_dir", 
        type=str,  
    ...
    extract_brepnet_data_from_step(step_path, output_path, mesh_dir, seg_dir, feature_list_path, num_workers=args.num_workers)