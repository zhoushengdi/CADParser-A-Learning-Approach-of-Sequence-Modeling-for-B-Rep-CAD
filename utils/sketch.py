...
class SketchBase(object):
    ...
    def normalize(self, size=256):
        ...
        scale = (size / 2  - 1) / cur_size # prevent potential overflow if data augmentation applied
    ...
    def denormalize(self, bbox_size, size=256):
        ...
        scale = bbox_size / (size / 2 - 1.0)
        self.transform(-np.array((size / 2.0, size / 2.0)), scale)
...
class Loop(SketchBase):
    ...
    ## 一个Profile对应一个(seq_N*16),其中在dim=0的角度看就是[SOL,Curve1, Curve2, Curve3, ..., EOS]
...
class Profile(SketchBase):
    ...
    ### max_n_loops:代表一个Sketch中Profile的最大个数    max_len_loop：代表一个Profile中Curve的最大个数
    ### output：shape(max_n_loops * max_len_loop, 16)，其中肯定以EOF结尾
