...
def file_chunk_stems(file_list, level=2):
    ## 根据数据保存的层级关系来提取文件名，level表示文件夹下直接是文件，2级表示会有子文件夹，目前设定最高2级
    stems = []
    for file in file_list:
        if level == 2:
            stems.append((str(file).split('/')[-2] + '/' + file.stem))
        else:
...
def get_train_test_lists_from_split(npz_folder, test_split):
    # files = [ (str(f).split('/')[-2] + '/' + f.stem) for f in npz_folder.glob("*/*.npz")]
...
def build_dataset_file(
    ...
    if validation_files is not None and test_files is not None:
        data = {
            "train": file_chunk_stems(train_files),
            "validation": file_chunk_stems(validation_files),
            "test": file_chunk_stems(test_files),
            "feature_standardization": standardization_data
        }
    else:
        data = {
            "train": file_chunk_stems(train_files,1),
            "feature_standardization": standardization_data
        }
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_test", type=str, default=None, help="Pathname to the file containing the train/test split")
    ...
    parser.add_argument("--output", type=str, required=True, help="json file(Pathname to save the generated dataset file)")
